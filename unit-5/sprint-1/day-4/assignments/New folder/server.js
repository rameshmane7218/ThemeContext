...
app.delete("/todos/:id", (req, res) => {
  ...
  fs.readFile("./data.json", { encoding: "utf-8" }, (err, data) => {
    ...
    const parsedData = JSON.parse(data);
    const deletedItem = parsedData.todos.filter((el) => el.id === Number(id));
    parsedData.todos = parsedData.todos.filter((el) => el.id !== Number(id));
...
app.put("/todos/:id", (req, res) => {
  ...
  fs.readFile("./data.json", { encoding: "utf-8" }, (err, data) => {
    ...
    const parsedData = JSON.parse(data);
    const updateData = parsedData.todos.filter((el) => el.id === Number(id));
